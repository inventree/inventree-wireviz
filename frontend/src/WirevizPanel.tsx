...
function WirevizBomRow({
  ...
  const partLink: string =
    !!row.sub_part && !!row.pn
      ? getDetailUrl(ModelType.part, row.sub_part, true)
      : '';
      ...
      <Table.Td key='col-prt'>
        {partLink ? (
          <Anchor
            href={partLink}
            onClick={(event: any) =>
              navigateToLink(partLink, context.navigate, event)
            }
          >
            {row.pn}
          </Anchor>
...
function WirevizPanel({ context }: { context: InvenTreePluginContext }) {
  ...
  const bomRows = useMemo(
    () => wirevizContext.wireviz_bom_data ?? [],
    [wirevizContext]
  );

  const wirevizDiagram = useMemo(
    () => wirevizContext.wireviz_svg_file ?? null,
    [wirevizContext]
  );

  const wirevizErrors = useMemo(
    () => wirevizContext.wireviz_errors ?? [],
    [wirevizContext]
  );

  const wirevizWarnings = useMemo(
    () => wirevizContext.wireviz_warnings ?? [],
    [wirevizContext]
  );

  const wirevizSource = useMemo(
    () => wirevizContext.wireviz_source_file ?? null,
    [wirevizContext]
  );

  const canEdit: boolean = useMemo(
    () => wirevizContext?.can_edit ?? false,
    [wirevizContext]
  );
              ...
              <Group justify='space-between'>
                ...
                {canEdit && (
                  <Menu position='bottom-end' withArrow>
                    ...
                    <Menu.Dropdown>
                      <Menu.Item
                        onClick={newDiagram.open}
                        leftSection={<IconUpload color='green' />}
                      >
                        Upload New Diagram
                      </Menu.Item>
                      {wirevizSource && (
                        <Menu.Item
                          onClick={deleteDiagram.open}
